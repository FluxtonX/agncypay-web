      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="rounded-xl border border-white/10 bg-black p-6">
            <p className="text-sm font-semibold text-[#8f8f8f]">Ready to Payout</p>
            <p className="mt-2 text-2xl font-bold">${paidInvoices.reduce((sum, inv) => sum + inv.amount * 0.85, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
            <p className="mt-1 text-xs text-[#8f8f8f]">{paidInvoices.length} invoices</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black p-6">
            <p className="text-sm font-semibold text-[#8f8f8f]">Processing</p>
            <p className="mt-2 text-2xl font-bold">$0.00</p>
            <p className="mt-1 text-xs text-[#8f8f8f]">No active batches</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-black p-6">
            <p className="text-sm font-semibold text-[#8f8f8f]">Recipients</p>
            <p className="mt-2 text-2xl font-bold">{new Set(paidInvoices.map(inv => inv.talentEmail)).size}</p>
            <p className="mt-1 text-xs text-[#8f8f8f]">Unique talent accounts</p>
          </div>
        </div>
        ...
        {paidInvoices.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-black p-12 text-center">
            <p className="text-lg font-semibold text-white">No payout batches yet</p>
            <p className="mt-2 text-sm text-[#8f8f8f]">Process invoices to create payout batches</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#8f8f8f] uppercase tracking-wider">Invoice</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#8f8f8f] uppercase tracking-wider">Talent</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-[#8f8f8f] uppercase tracking-wider">Amount</th>
              ...
              <tbody>
                {paidInvoices.map((invoice) => (
                  <tr key={invoice.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="px-6 py-4 text-sm">{invoice.talent}</td>
                    <td className="px-6 py-4 text-sm font-semibold">${(invoice.amount * 0.85).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-900/30 text-green-400 border border-green-800/30">
                        {invoice.talentPayoutStatus === "disbursed" ? "Disbursed" : "Ready"}
                      </span>
                    </td>
                  </tr>
                ))}
