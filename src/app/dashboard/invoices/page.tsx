...
export default function InvoicesPortalPage() {
  ...
  // Fetch real invoices from Firestore for agency users
  useEffect(() => {
    if (workspaceType === "agency" && state.user?.email) {
      const unsubscribe = subscribeInvoicesByAgency(state.user.email, (firestoreInvoices: FirestoreInvoice[]) => {
        // Map Firestore invoices to the InvoiceRow format
        const mappedInvoices = firestoreInvoices.map((inv) => ({
          id: inv.id,
          agency: inv.agency,
          campaign: inv.campaign,
          amount: inv.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
          fees: (inv.amount * 0.015).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
          status: inv.status === "pending" ? "Pending" : inv.status === "paid" ? "Paid" : "Processing",
          due: inv.due,
        }));
  ...
  }, [workspaceType, state.user?.email]);
