...
export default function AgencyDashboardPage() {
  ...
  const workspaceType = "agency" as "agency" | "brand"; // Fixed to agency
  ...
  const liveFunctionalInvoices: any[] = userFilteredWidgetInvoices.map(inv => {
    ...
    const talentAvatar = (talentUser as any)?.profileImage || ""; // Empty string if no profile image
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setInvoices((prev) => {
            ...
            const targetInvoice = prev.find((inv) => inv.id === id);
            if (targetInvoice) {
              const talentSplit = targetInvoice.splitPool.splits.find((s: any) => s.role === "Talent");
              const agencySplit = targetInvoice.splitPool.splits.find((s: any) => s.role === "Agency");
    ...
    <main className="min-h-screen bg-[#000000] text-white flex flex-col font-sans antialiased selection:bg-white selection:text-black relative">
      ...
      <header className="border-b border-white/20 bg-black/90 sticky top-0 z-50 shadow-sm backdrop-blur">
        <div className="max-w-[1520px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {registeredTalents.some(t => t.email === state.user?.email) && (
              <>
                <button
                ...
                {workspaceType === "agency" && (
                  <div>
                    ...
                    <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                      {activeInvoice.splitPool.splits.map((split: any) => (
                        <div
                          key={split.name}
